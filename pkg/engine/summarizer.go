...
func (s *Summarizer) callAPI(ctx context.Context, req chatRequest) (*FileEnrichment, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("status %d: %s", resp.StatusCode, previewBody(rawBody))
	}

	var chat chatResponse
	if err := json.Unmarshal(rawBody, &chat); err != nil {
		return nil, fmt.Errorf("decode response: %w: %s", err, previewBody(rawBody))
	}
	if len(chat.Choices) == 0 || chat.Choices[0].Message.Content == "" {
		return nil, fmt.Errorf("empty response: %s", previewBody(rawBody))
	}

	content := stripCodeFence(chat.Choices[0].Message.Content)
	var parsed enrichJSON
	if err := json.Unmarshal([]byte(content), &parsed); err != nil {
		return nil, fmt.Errorf("parse JSON: %w: %s", err, previewBody([]byte(content)))
	}
...
// stripCodeFence removes markdown code fences (e.g. ```json ... ```) that some
// models emit even when json_object response format is requested.
...
// previewBody returns a single-line, truncated excerpt of a response body
